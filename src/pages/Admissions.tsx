...
const applicationSchema = z.object({
  ...
  documents: z.object({
    ...
    reportCardUrl: z.string().min(1, 'Latest Report Card is required'),
    guardianIdUrl: z.string().min(1, 'Guardian ID is required'),
    healthCertificateUrl: z.string().min(1, 'Health Certificate is required'),
    doctorDeclarationUrl: z.string().min(1, 'Doctor Declaration is required'),
    transferDocumentUrl: z.string().optional(),
  }),
});
...
type DocKey =
  ...
  | 'doctorDeclarationUrl'
  | 'transferDocumentUrl';

type DocSpec = { id: DocKey; label: string; required: boolean };
...
const DOCS: DocSpec[] = [
  { id: 'birthCertificateUrl', label: 'Birth Certificate', required: true },
  { id: 'reportCardUrl', label: 'Latest Report Card', required: true },
  { id: 'guardianIdUrl', label: 'Guardian ID / Passport', required: true },
  { id: 'healthCertificateUrl', label: 'Health / Immunization Certificate', required: true },
  { id: 'doctorDeclarationUrl', label: 'Doctor Declaration', required: true },
  { id: 'transferDocumentUrl', label: 'Transfer Document (If applicable)', required: false },
];
...
export default function Admissions() {
  ...
  const [selectedFiles, setSelectedFiles] = useState<Partial<Record<DocKey, File>>>({});
  ...
  const uploadOne = async (applicationNumber: string, docKey: DocKey, file: File) => {
    const ext = file.name.split('.').pop() || 'file';
    const path = `applications/${applicationNumber}/${docKey}-${Date.now()}.${ext}`;
  ...
  const onSubmit = async (data: ApplicationForm) => {
    ...
    try {
      ...
      // Upload selected files to Storage, then set URLs into documents.*
      const docsPayload: Record<string, string | undefined> = { ...data.documents };

      for (const doc of DOCS) {
        const file = selectedFiles[doc.id];
        if (!file) {
          if (doc.required) {
            throw new Error(`Missing required file: ${doc.label}`);
          }
          continue;
        }
        const url = await uploadOne(applicationNumber, doc.id, file);
        docsPayload[doc.id] = url;
        setValue(`documents.${doc.id}` as any, url);
      }
      ...
      <div className="max-w-4xl mx-auto">
                  ...
                  <motion.div
                    ...
                    className={cn(
                      ...
                      isCompleted
                        ...
                        : isActive
                        ? 'bg-navy-900 border-navy-900 text-white ring-4 ring-navy-100 shadow-lg shadow-navy-900/20'
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 md:p-12">
          <AnimatePresence mode="wait">
            {/* steps 1-3 unchanged for brevity (kept as-is by reuse from original file) */}
            {step === 1 && (
            ...
            {step === 4 && (
              <motion.div key="step4" initial= opacity: 0, x: 40  animate= opacity: 1, x: 0  exit= opacity: 0, x: -40  className="space-y-6">
                ...
                <p className="text-sm text-gray-600 mb-6">
                  Please upload clear copies of the following required documents. Files are securely uploaded when you submit the application.
                </p>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {DOCS.map((doc) => {
                    const fieldName = `documents.${doc.id}` as const;
                    const urlValue = watch(fieldName as any);
                    const hasFile = !!selectedFiles[doc.id];
                      ...
                      <div key={doc.id} className="space-y-2">
                        ...
                        <div
                          className={cn(
                            ...
                            hasFile || urlValue ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-navy-300',
                          ...
                          {isUploading ? (
                            <div className="w-full px-4 space-y-3">
                              ...
                              <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                                <motion.div initial= width: 0  animate={{ width: `${progress}%` }} transition= duration: 0.2  className="bg-navy-900 h-full rounded-full" />
                              </div>
                            </div>
                          ) : hasFile || urlValue ? (
                            <>
                              ...
                              <span className="text-xs font-medium text-green-700">Ready to upload</span>
                              <button
                                type="button"
                                onClick={() => {
                            ...
                            <>
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  // store a placeholder so the step validates; it will be overwritten with a URL on submit
            ...
            {step === 5 && (
              <motion.div key="step5" initial= opacity: 0, x: 40  animate= opacity: 1, x: 0  exit= opacity: 0, x: -40  className="space-y-6">
          ...
          <div className="mt-12 flex justify-between pt-8 border-t border-gray-100">
            {step > 1 ? (
              <button type="button" onClick={prevStep} className="flex items-center gap-2 px-6 py-3 text-gray-600 font-bold hover:text-navy-900 transition-colors">
            ...
            {step < 5 ? (
              <button type="button" onClick={nextStep} className="flex items-center gap-2 px-8 py-3 bg-navy-900 text-white rounded-lg font-bold hover:bg-navy-800 transition-all shadow-lg shadow-navy-900/20">
