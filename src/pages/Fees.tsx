...
import { CreditCard, FileText, Download, CheckCircle, AlertCircle, Info, Check, User, Mail, Hash } from 'lucide-react';
...
export default function Fees() {
  ...
  const canGenerateDocs = useMemo(() => !!logoDataUrl, [logoDataUrl]);
  ...
  const addLogoToPdf = (doc: jsPDF) => {
    ...
    // logo on header area
    try {
      doc.addImage(logoDataUrl, 'JPEG', 160, 8, 35, 24);
    } catch {
      // If addImage fails (format mismatch), just skip.
    }
  };
  ...
  const generateInvoice = (invoiceNum: string, isReceipt = false) => {
    ...
    doc.text(
      isReceipt ? 'Receipt generated by the website.' : 'Please email proof of payment to info@poweroflove.edu.za',
      105,
      280,
      { align: 'center' }
    );
  ...
  const generateStationeryPDF = () => {
    ...
    if (!logoDataUrl) {
      alert('Please wait a moment for the school logo to load, then try again.');
      return;
    }
  ...
  const completePayFastCheckout = async () => {
    // NOTE: Demo-mode only. This does not communicate with PayFast.
    setIsProcessing(true);
    try {
      const invoiceNum = `DEMO-${Date.now().toString().slice(-6)}`;
  ...
  if (paymentSuccess) {
    return (
      <div className="max-w-2xl mx-auto py-24 px-4 text-center">
        ...
        <h1 className="text-4xl font-bold text-navy-900 mb-4">{lastMethod === 'PayFast' ? 'Payment Completed (Demo)' : 'Payment Initiated!'}</h1>
        ...
        {lastMethod === 'EFT' ? (
          ...
          <div className="bg-yellow-50 p-6 rounded-xl border border-yellow-100 mb-8 text-left">
            <h3 className="font-bold text-yellow-900 mb-2">Demo payment</h3>
            <p className="text-sm text-yellow-800">This PayFast button is running in demo mode and does not process a real transaction.</p>
          </div>
        )}
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="bg-navy-900 p-6 text-white text-center">
                ...
                <h3 className="text-xl font-bold">{PAYFAST_DEMO_MODE ? 'PayFast Demo Checkout' : 'Secure Checkout'}</h3>
                <p className="text-navy-300 text-sm">{PAYFAST_DEMO_MODE ? 'Demo only. No real transaction will be processed.' : 'Redirecting to PayFast'}</p>
              </div>
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="p-4 bg-blue-50 rounded-xl border border-blue-100 flex gap-3">
                  ...
                  <p className="text-[10px] text-blue-800 leading-relaxed">
                    This is a demo environment. The app will generate a receipt and save a demo payment record, but it will not communicate with PayFast.
                  </p>
                </div>

                <button
                  onClick={completePayFastCheckout}
                  disabled={isProcessing || !canGenerateDocs}
                  className="w-full py-4 bg-navy-900 text-white rounded-xl font-bold hover:bg-navy-800 transition-all flex items-center justify-center gap-2 shadow-lg shadow-navy-900/20 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {isProcessing ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : 'Confirm (Demo) and Generate Receipt'}
                </button>
      ...
      <div className="max-w-6xl mx-auto">
          ...
          {GRADES.map((grade) => (
            <button
              ...
              className={cn(
                ...
                selectedGrade === grade ? 'bg-navy-900 text-white border-navy-900 shadow-lg shadow-navy-900/20' : 'bg-white text-gray-600 border-gray-200 hover:border-navy-900 hover:text-navy-900'
              )}
            >
        ...
        <AnimatePresence mode="wait">
          {selectedGrade ? (
            <motion.div key={selectedGrade} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {/* Fee Details */}
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                  ...
                  <button onClick={generateStationeryPDF} className="mt-8 flex items-center gap-2 text-navy-900 font-bold hover:underline">
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden h-fit sticky top-24">
                <div className="bg-navy-900 p-6 text-white">
                  ...
                  <p className="text-navy-200 text-xs mt-1">{PAYFAST_DEMO_MODE ? 'PayFast is currently demo mode.' : 'PayFast live payments.'}</p>
                </div>
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        {isProcessing ? (
                          ...
                          <>
                            <CreditCard className="w-5 h-5" /> Pay Online (PayFast Demo)
                          </>
                        )}
                      </button>
                      <button
